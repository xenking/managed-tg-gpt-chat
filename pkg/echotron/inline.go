...
func (a API) AnswerInlineQuery(ctx context.Context, inlineQueryID string, results []InlineQueryResult, opts *InlineQueryOptions) (res APIResponseBase, err error) {
	vals := make(url.Values)

	jsn, _ := json.Marshal(results)
	vals.Set("inline_query_id", inlineQueryID)
	vals.Set("results", string(jsn))
	return res, a.client.get(ctx, a.base, "answerInlineQuery", addValues(vals, opts), &res)
}
