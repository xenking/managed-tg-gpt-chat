...
func (a API) SendInvoice(ctx context.Context, chatID int64, title, description, payload, currency string, prices []LabeledPrice, opts *InvoiceOptions) (res APIResponseMessage, err error) {
	...
	return res, a.client.get(ctx, a.base, "sendInvoice", addValues(vals, opts), &res)
}

// CreateInvoiceLink creates a link for an invoice.
func (a API) CreateInvoiceLink(ctx context.Context, title, description, payload, currency string, prices []LabeledPrice, opts *CreateInvoiceLinkOptions) (res APIResponseBase, err error) {
	vals := make(url.Values)
	...
	vals.Set("currency", currency)
	vals.Set("prices", string(p))
	return res, a.client.get(ctx, a.base, "createInvoiceLink", addValues(vals, opts), &res)
}

// AnswerShippingQuery is used to reply to shipping queries.
// If you sent an invoice requesting a shipping address and the parameter is_flexible was specified,
// the SessionHandler API will send an Update with a shipping_query field to the bot.
func (a API) AnswerShippingQuery(ctx context.Context, shippingQueryID string, ok bool, opts *ShippingQueryOptions) (res APIResponseBase, err error) {
	vals := make(url.Values)

	vals.Set("shipping_query_id", shippingQueryID)
	vals.Set("ok", btoa(ok))
	return res, a.client.get(ctx, a.base, "answerShippingQuery", addValues(vals, opts), &res)
}

// AnswerPreCheckoutQuery is used to respond to such pre-checkout queries.
// Once the user has confirmed their payment and shipping details,
// the SessionHandler API sends the final confirmation in the form of an Update with the field pre_checkout_query.
// NOTE: The SessionHandler API must receive an answer within 10 seconds after the pre-checkout query was sent.
func (a API) AnswerPreCheckoutQuery(ctx context.Context, preCheckoutQueryID string, ok bool, opts *PreCheckoutOptions) (res APIResponseBase, err error) {
	vals := make(url.Values)

	vals.Set("pre_checkout_query_id", preCheckoutQueryID)
	vals.Set("ok", btoa(ok))
	return res, a.client.get(ctx, a.base, "answerPreCheckoutQuery", addValues(vals, opts), &res)
}

// GetStarTransactions returns the bot's Telegram Star transactions in chronological order.
func (a API) GetStarTransactions(ctx context.Context, opts *StarTransactionsOptions) (res APIResponseStarTransactions, err error) {
	return res, a.client.get(ctx, a.base, "getStarTransactions", urlValues(opts), &res)
}

// RefundStarPayment refunds a successful payment in Telegram Stars.
func (a API) RefundStarPayment(ctx context.Context, userID int64, telegramPaymentChargeID string) (res APIResponseBool, err error) {
	vals := make(url.Values)

	vals.Set("user_id", itoa(userID))
	vals.Set("telegram_payment_charge_id", telegramPaymentChargeID)
	return res, a.client.get(ctx, a.base, "refundStarPayment", vals, &res)
}
