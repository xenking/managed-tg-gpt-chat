...
// Update wraps echotron.Update and stores some additional data.
type Update struct {
	*echotron.Update
	*echotron.API
...
func NewUpdate(update *echotron.Update, bot echotron.API, botSelf *echotron.User) *Update {
	return &Update{
		Update:  update,
		API:     &bot,
		BotSelf: botSelf,
...
func (u *Update) EffectiveUser() *echotron.User {
	if u.Message != nil {
...
func (u *Update) EffectiveChat() *echotron.Chat {
...
func (u *Update) EffectiveMessage() *echotron.Message {
	candidates := []*echotron.Message{u.Message, u.EditedMessage, u.ChannelPost, u.EditedChannelPost}
